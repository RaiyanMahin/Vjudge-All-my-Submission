...
int query(int node, int lo, int hi, int i, int j, int x){
    ...
    if(lo==hi) return (bool)(tree[node]%x);
    int Left=node*2;
    int Right=node*2+1;
    int mid=(lo+hi)/2;
    ...
    if (lo>=i && hi<=j) {
        ...
        if(tree[Left]%x && tree[Right]%x) return 2;
        else if(tree[Left]%x) return query(Left,lo,mid,i,j,x);
        else if(tree[Right]%x) return query(Right,mid+1,hi,i,j,x);
    }
    return query(Left,lo,mid,i,j,x)+query(Right,mid+1,hi,i,j,x);
}
